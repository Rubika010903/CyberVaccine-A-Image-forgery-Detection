...
@app.route('/cyber_vaccine', methods=['GET', 'POST'])
def cyber_vaccine():
    msg=""
    if request.method=='POST':
        uname=request.form['uname']
...
#Photo Forgery Detection Network (PFDNet) using INN
#Invertible Neural Network (INN)-detect and localize the tampered regions of the image
class PFDNet:
...
#Run Length Encoding
#(Lossless Recovery-RLE to validate the recovery of the image)
...
def userhome():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            ##
            rfile="R"+str(maxid)+".txt"
            image_path = "static/upload/"+file_name  # Change to your image file
            image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)

            # Encode the image using RLE
            encoded_data = rle_encode(image)
            with open("static/test/"+rfile, "w") as file:
                json.dump(encoded_data, file)

            with open("static/test/"+rfile, "r") as file:
            ...
            #cv2.imwrite("static/test/DR"+file_name, decoded_image)
            ##
